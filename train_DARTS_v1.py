...
class MixedPrecisionQATLinearEnhanced(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        ref_x = x_flat.detach()    
        q_weight = torch.zeros_like(self.weight)
        q_x = torch.zeros_like(x_flat)
        err_w = self.weight.new_zeros(())
        err_x = x_flat.new_zeros(())
        for p, q in zip(p_w, self.quantizers_w):
            qw = q(self.weight)
            q_weight = q_weight + p * qw
            if self.collect_error_stats:
                err_w = err_w + p * F.mse_loss(qw, ref_w)
        for p, q in zip(p_a, self.quantizers_a):
            qx = q(x_flat)
            q_x = q_x + p * qx
            if self.collect_error_stats:
                err_x = err_x + p * F.mse_loss(qx, ref_x)
...
def main() -> None:
    ...
    ds_val = ds_val.skip(2000).take(200)
    ...
    def tokenize_fn(examples: Dict[str, List[str]]) -> Dict[str, List[List[int]]]:
        return tokenizer(examples["text"], truncation=True, padding=False, max_length=5)
